            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                  　<div class="small text-muted">{{ \Str::limit($latest_post->created_at) }}</div>
                    ...
                    <!-- イイねボタン -->
                    {{--<span>
                        <img src="{{ asset("img/likeButton.jpg")}}" width="20px">
                        <!-- もし$likeがあれば = ユーザーが「イイね」をしていたら -->
                        @if($like)
                        <!-- 「イイね」取消用ボタンを表示 -->
                            <a href="{{ route('unlike', $latte) }}" class="btn btn-success btn-sm">
                                イイね
                                <!-- 「イイね」の数を表示 -->
                                <span class="badge">
                                    {{ $latte->likes->count() }}
                        ...
                        @else
                        <!-- まだユーザーが「イイね」をしていなければ、「イイね」ボタンを表示 -->
                    ...
                    @if($latest_post->likes->count() > 0)
                         <!-- 「イイね」取消用ボタンを表示 -->
                        <a href="{{ route('unlike', $latest_post) }}" class="btn btn-success btn-sm">
                            イイね
                            <!-- 「イイね」の数を表示 -->
                            <span class="badge">
